/**
 * Probe Grid — Net Worth Edition
 *
 * Replaces Seniority with Net Worth (13 brackets).
 * Gender(2) x Homeowner(2) x Married(2) x NetWorth(13) x Children(2) x Age(6) = 1,248 combos
 *
 * Usage:
 *   API_BASE=http://localhost:3001/api npx tsx backend/src/scripts/filter-explorer/probe-grid-networth.ts
 *   API_BASE=http://localhost:3001/api npx tsx backend/src/scripts/filter-explorer/probe-grid-networth.ts --dry-run
 *   API_BASE=http://localhost:3001/api npx tsx backend/src/scripts/filter-explorer/probe-grid-networth.ts --limit 10
 */
...
class GridCheckpoint {
  ...
  constructor(mode: string) {
    ...
    this.cpPath = path.join(dir, `checkpoint-grid-networth-${mode}.json`);
    ...
    if (fs.existsSync(this.cpPath)) {
      ...
      this.resultPath = path.join(dir, `grid-networth-${mode}-${ts}.ndjson`);
...
async function main() {
  ...
  console.log('======================================================');
  console.log(`  PROBE GRID — NET WORTH (${mode.toUpperCase()})`);
  ...
  console.log('[grid-nw] Pre-warming VacuumEngine...');
  ...
  console.log('[grid-nw] Pre-warm complete.\n');

  console.log('[grid-nw] Initializing audience...');
  const initResult = await api('/vacuum/init', { name: 'Grid Net Worth' });
  if (!initResult.success) throw new Error('Init failed: ' + JSON.stringify(initResult));
  const audienceId = initResult.audienceId;
  console.log(`[grid-nw] Audience ready: ${audienceId}\n`);
  ...
  for (const combo of combos) {
    ...
    if (consecutiveFailures >= 5) {
      console.error(`\n[grid-nw] 5 consecutive failures — cooling down 30s\n`);
      await sleep(30000);
      consecutiveFailures = 0;
    }
  ...
  console.log('\n======================================================');
  console.log('  GRID NET WORTH RESULTS');
  console.log('======================================================\n');
