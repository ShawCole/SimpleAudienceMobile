/**
 * Probe Grid — Income Range Edition
 *
 * Replaces Seniority with Income Range (9 brackets).
 * Gender(2) x Homeowner(2) x Married(2) x Income(9) x Children(2) x Age(6) = 864 combos
 *
 * Usage:
 *   API_BASE=http://localhost:3001/api npx tsx backend/src/scripts/filter-explorer/probe-grid-income.ts
 *   API_BASE=http://localhost:3001/api npx tsx backend/src/scripts/filter-explorer/probe-grid-income.ts --dry-run
 *   API_BASE=http://localhost:3001/api npx tsx backend/src/scripts/filter-explorer/probe-grid-income.ts --limit 10
 */
...
class GridCheckpoint {
  ...
  constructor(mode: string) {
    ...
    this.cpPath = path.join(dir, `checkpoint-grid-income-${mode}.json`);
    ...
    if (fs.existsSync(this.cpPath)) {
      ...
      this.resultPath = path.join(dir, `grid-income-${mode}-${ts}.ndjson`);
...
async function main() {
  ...
  console.log('======================================================');
  console.log(`  PROBE GRID — INCOME RANGE (${mode.toUpperCase()})`);
  ...
  console.log('[grid-inc] Pre-warming VacuumEngine...');
  ...
  console.log('[grid-inc] Pre-warm complete.\n');

  console.log('[grid-inc] Initializing audience...');
  const initResult = await api('/vacuum/init', { name: 'Grid Income Range' });
  if (!initResult.success) throw new Error('Init failed: ' + JSON.stringify(initResult));
  const audienceId = initResult.audienceId;
  console.log(`[grid-inc] Audience ready: ${audienceId}\n`);
  ...
  for (const combo of combos) {
    ...
    if (consecutiveFailures >= 5) {
      console.error(`\n[grid-inc] 5 consecutive failures — cooling down 30s\n`);
      await sleep(30000);
      consecutiveFailures = 0;
    }
  ...
  console.log('\n======================================================');
  console.log('  GRID INCOME RANGE RESULTS');
  console.log('======================================================\n');
